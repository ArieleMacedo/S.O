import { limparTela, limparTelaEnter } from "./utils";

export class Tarefa {
  private _id: number = 1;
  private _ingressoOriginal: number;
  public readonly duracao: number;

  private _executado: number = 0;
  private _termino: number | null = null;
  public ingresso: number;

  constructor(ingresso: number, duracao: number) {
    this.ingresso = ingresso;
    this._ingressoOriginal = ingresso;
    this.duracao = duracao;
  }

  get id() {
    return this._id;
  }
  set id(umId: number) {
    this._id = umId;
  }
  get ingressoOriginal(): number {
    return this._ingressoOriginal;
  }
  get restante(): number {
    return this.duracao - this._executado;
  }
  get termino() {
    return this._termino;
  }

  registrarExecucao(t: number): void {
    this._executado += t;
  }

  concluir(tempo: number): void {
    this._termino = tempo;
  }

  terminou(): boolean {
    return this._executado >= this.duracao;
  }
}

export class Escalonador {
  private _tarefas: Tarefa[] = [];
  private _filaProntas: Tarefa[] = [];
  private _quantum: number;
  private _trocaDeContexto: number;
  private _idTarefaAtual = 1;
  private _relogio = 0;

  constructor(quantum: number, tc: number) {
    this._quantum = quantum;
    this._trocaDeContexto = tc;
  }

  adicionarTarefa(t: Tarefa): void{
    t.id = this._idTarefaAtual++;
    this._tarefas.push(t);
  }

  private carregarProntas(): void{
    const prontas = this._tarefas.filter((t) => t.ingresso <= this._relogio);
    this._filaProntas.push(...prontas);
    this._tarefas = this._tarefas.filter((t) => t.ingresso > this._relogio);
  }

  executar() {
    this._tarefas.sort((a, b) => a.ingresso - b.ingresso);
    while (true) {
      this.carregarProntas();

      if (this._filaProntas.length === 0) {
        if (this._tarefas.length === 0) break;
        this._relogio = this._tarefas[0]!.ingresso;
        continue;
      }

      this._filaProntas.sort((a, b) => a.ingresso - b.ingresso);
      const tarefa = this._filaProntas.shift()!;

      const tempoExec = Math.min(this._quantum, tarefa.restante);
      tarefa.registrarExecucao(tempoExec);
      this._relogio += tempoExec;

      this.carregarProntas();

      if (tarefa.terminou()) {
        tarefa.concluir(this._relogio);
        if (this._tarefas.length > 0 || this._filaProntas.length > 0) {
          this._relogio += this._trocaDeContexto;
        }
      } else if (this._tarefas.length === 0 && this._filaProntas.length === 0) {
        const tempoExec = tarefa.restante;
        tarefa.registrarExecucao(tempoExec);
        this._relogio += tempoExec;
        tarefa.concluir(this._relogio);
        break;
      } else {
        tarefa.ingresso = this._relogio;
        this._relogio += this._trocaDeContexto;
        this.carregarProntas();
        this._filaProntas.push(tarefa);
      }
    }
  }

  gerarResumo(tarefas: Tarefa[]) {
    console.log("\n--- Resumo Final ---");

    console.log("ID  | Ing | Dur | Término | Vida | Espera");
    console.log("------------------------------------------");

    let somaVida = 0;
    let somaEspera = 0;

    for (const t of tarefas) {
      const vida = t.termino! - t.ingressoOriginal;
      const espera = vida - t.duracao;

      somaVida += vida;
      somaEspera += espera;

      console.log(
        `${String(t.id).padEnd(3)}| ` +
          `${String(t.ingressoOriginal).padEnd(4)}| ` +
          `${String(t.duracao).padEnd(4)}| ` +
          `${String(t.termino).padEnd(8)}| ` +
          `${String(vida).padEnd(5)}| ` +
          `${String(espera).padEnd(6)}`
      );
    }

    console.log("------------------------------------------");
    console.log(
      `Tempo médio de vida: ${(somaVida / tarefas.length).toFixed(2)}`
    );
    console.log(
      `Tempo médio de espera: ${(somaEspera / tarefas.length).toFixed(2)}`
    );
  }
}

let esc: Escalonador = new Escalonador(5,0);

let t1: Tarefa = new Tarefa(0,5);
let t2: Tarefa = new Tarefa(0,2);
let t3: Tarefa = new Tarefa(1,4);
let t4: Tarefa = new Tarefa(3,1);
let t5: Tarefa = new Tarefa(5,2);

esc.adicionarTarefa(t1);
esc.adicionarTarefa(t2);
esc.adicionarTarefa(t3);
esc.adicionarTarefa(t4);
esc.adicionarTarefa(t5);
esc.executar()
esc.gerarResumo([t1,t2,t3,t4,t5])



